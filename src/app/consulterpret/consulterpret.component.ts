...
@Component({
  ...
  // providers: [ConsulterpretServiceWeb]
...
export class ConsulterpretComponent implements OnInit {
  ...
  ngOnInit() {
  	...
  	this.consulterpretServiceWeb.consulterpret(this.token).then(consulterpretserviceList => {
        this.consulterpret = consulterpretserviceList;
        console.log(JSON.stringify(this.consulterpret));
        this.loading = false ;
      });
  }
